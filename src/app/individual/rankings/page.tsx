...
export default async function IndividualRankingsPage({ searchParams }: PageProps) {
  const params = await searchParams;
  const market = parseMarketFilter(params.market);
  const [meta, rankings] = await Promise.all([
    getMinimalInvestorDashboardMeta("INDIVIDUAL"),
    getFullPeriodInvestorRankings("INDIVIDUAL", market, 500),
  ]);
    ...
    <AppShell hasData={meta.hasData}>
      {!meta.hasData ? (
        ...
        <InvestorFullRankingsView
          ...
          title="개인 매매 동향"
          pathname="/individual/rankings"
          backHref={`/individual${backQs}`}
        />
      )}
    </AppShell>
  );
